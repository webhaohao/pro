<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:69:"C:\xampp\htdocs\pro\public/../application/index\view\index\index.html";i:1559316053;}*/ ?>
<!DOCTYPE html>
<html>
<head>
<title>首页</title>
						...
						<el-table-column v-for="(v,i) in master_user.columns" :prop="v.field" :label="v.title" :width="v.width">
								<template slot-scope="scope">
										<el-form-item v-if="scope.row.isSet" :rules="rules[v.field]" :prop="'data.'+scope.$index+'.'+[v.field]" >
												<el-input size="mini" :ref="scope.row.id" placeholder="请输入内容" v-model="scope.row[v.field]" @keydown.native="nextFocus(scope.row, scope.$index, $event)" ></el-input>
										</el-form-item>
    ...
    var app = new Vue({
        ...
        methods: {
					addInfo(){
						...
						// }
						let j = {id:this.master_user.data.length,"serialnum": "", "serialdes": "", "partIntr": "", "count": "", "remarks": "", "isSet": true, "_temporary": true };
					...
					nextFocus(row, index, e){
						if([Object.keys(this.$refs)[index+1]]!="ruleForm"){
							this.$refs[row.id][index].blur();
